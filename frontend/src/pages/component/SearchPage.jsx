...
const SearchPage = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/api/products/search?query=${encodeURIComponent(query)}`, {
        headers: {
  ...
  return (
    <div className="flex items-center justify-center transition-all duration-1000 ease-in-out relative">
      <div className="relative z-10 text-center w-full flex flex-col min-h-screen ">
          ...
          <div className="flex flex-row gap-4">
            {isLoggedIn ? (
              <>
                ...
                {user && (
                  <span className="text-neutral-300 mr-2 mt-1 font-semibold" onClick={()=> navigate("/market")}>
                    Marketplace
                  </span>
                )}
                <button 
        ...
        >
          <h1 className="text-7xl font-medium mb-8 text-[#2f3727]">
